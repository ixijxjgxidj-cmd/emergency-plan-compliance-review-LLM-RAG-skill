...
- `./output/review_config.json`（本次所用挡位，决定取材范围）
...
0. **本次审查挡位与收敛漏斗（必须置于报告最前，任何挡位都不得省略）**：声明 `review_config.json` 的挡位与四个开关取值，并给出完整漏斗表——原始候选数 → 聚合后主张数 → A/B 级门禁后 → 全文反证后 → 最终交付数，每一级注明减少原因与条数。同时给出一句话：「若切至 L3 严格档将剩 N 条；若切至 L0 原始档将有 M 条」，让读者知道当前看到的数字处在什么位置。
...
### 取材范围：批注恒按 L3 标准，与挡位无关

无论 `review_config.json` 是哪个挡位，进入 `plan_annotated.docx` 批注的问题必须同时满足：依据在知识库内、`article` 与 `clause_text` 齐备、`quoted_text` 可定位、`suggestion` 非空、未被 5C2 判 `refuted`。

带 `basis_outside_kb` / `citation_unverifiable` / `incomplete_fields` / `refuted_by` 标记的问题**不进批注**，改列入 `issue_list.docx` 的「候选问题（未纳入批注）」附录表，每条注明未纳入的原因。

理由：批注是给评审专家直接用的交付件。实跑那次 52 条批注里 34 条依据不在库、还写着「依据来源存疑」，等于把不成立的结论递到专家面前。**分析产物全量，交付产物严格**——低挡位放宽的是分析清单，不是批注。
...
## issue_list.docx 的候选问题附录

正表之后另起一张表：「候选问题（未纳入批注）」，列：问题编号、所在章节、问题类型、问题描述、未纳入原因（`basis_outside_kb` / `citation_unverifiable` / `incomplete_fields` / `refuted_by` 之一，用中文写明）、若为 `refuted_by` 则给出落实位置的条款号与原文摘录。

这张表的作用是让审阅者能反查：哪些命中被判定为不成立、依据在哪、是不是误剔。**L0/L1/L2 挡位下这张表通常比正表长，这是正常的**，不得因为"太长"而截断或只给数字。
