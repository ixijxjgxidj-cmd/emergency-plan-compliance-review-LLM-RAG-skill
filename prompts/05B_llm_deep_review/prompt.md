...
两种模式在**派发方式**上也不同，这是全流程唯一的例外：

- **模式 A**：本阶段本身由主流程派发的子智能体执行，它再按 batch 二级派发子智能体。
- **模式 B**：本阶段**不派子智能体**，由执行者直接按 `model_config.json` 调用外部 API 逐条审查——既然用户指定了那个模型，就必须由它来审，套子智能体等于换掉了模型。
